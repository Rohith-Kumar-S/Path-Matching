...
if st.session_state.execute:
    ...
    source_index = _graph_gen.get_index_from_coordinates(source_coords)
    target_index = _graph_gen.get_index_from_coordinates(target_coords)
    ...
    match st.session_state.algorithm:
        case "Breadth-First Search":
            ...
            res, st.session_state.search_coords = AlgorithmsImpl().bfs(graph[0])
            print("bfs: ", res, "len", len(st.session_state.search_coords))
        case "Dijkstra":
            graph = list(blocks.values())
            res, st.session_state.search_coords = AlgorithmsImpl().a_star(graph, target_coords = (300,320, 600, 620), include_heuristic=False)
            print("dijkstra: ", res, "len", len(st.session_state.search_coords))
        case "A*":
            graph = list(blocks.values())
            res, st.session_state.search_coords = AlgorithmsImpl().a_star(graph, target_coords = (300,320, 600, 620), heuristic = st.session_state.heuristic.lower())
